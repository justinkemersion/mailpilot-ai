...
**An AI-driven inbox copilot** that reads intent—not just metadata—to tame Gmail. MailPilot pairs a **Next.js dashboard** (auth, Gmail linking, history, controls) with a **Python worker** (classification, Gmail actions, OpenAI) coordinated through **Flux** (PostgreSQL + PostgREST).
...
```text
mailpilot-ai/
├── mailpilot-runner/   # Python engine (CLI, Gmail, OpenAI, Flux persistence)
├── mailpilot-web/      # Next.js app (Auth.js, Google OAuth UX, dashboard, Flux client)
├── AGENTS.md           # Agent entrypoint (links to backlog, web rules, architecture)
├── BACKLOG.md          # Session-agnostic deferred work (for humans + agents)
├── .github/            # CI workflows
└── LICENSE
```

**Deferred work:** see [BACKLOG.md](BACKLOG.md). **Agent entrypoint:** [AGENTS.md](AGENTS.md).

### `mailpilot-web` — control plane

**Next.js** (App Router, TypeScript, Tailwind). Handles **Auth.js** sign-in, the **Google OAuth** flow that stores Gmail refresh tokens in `accounts`, the **dashboard** (history, undo, “Process inbox”), and **API routes** that read/write **Flux**. See [`mailpilot-web/README.md`](mailpilot-web/README.md) and [`mailpilot-web/ARCHETECTURE.md`](mailpilot-web/ARCHETECTURE.md).

### `mailpilot-runner` — execution engine

**Python** worker: **Gmail API**, **OpenAI** classification, label/archive actions, and **Flux** as the system of record (`accounts`, `processed_emails`, `run_jobs`). CLI commands include `run`, `run-once`, `watch-jobs` (job queue consumer for the dashboard), `history`, and `flux-check`. Full setup: [`mailpilot-runner/README.md`](mailpilot-runner/README.md).

---

## Standalone runner + web app (why this pattern?)

MailPilot deliberately splits **orchestration and UX** (Next.js) from **email processing** (Python). They communicate **only through Flux** (Postgres + PostgREST): the web app inserts **jobs** and **rows**; the runner **claims jobs**, reads tokens, calls Gmail/OpenAI, and writes results. Nothing requires the two processes to share a host.

### How it works in this repo

1. Users sign in (Auth.js) and connect Gmail in the **web app**; refresh tokens live in Flux `api.accounts`.
...
4. **History** is stored in **`processed_emails`**; the dashboard reads it via Flux with the user’s session subject. Server routes use the **Flux service token** where appropriate—still no OpenAI in Next.js.
...
| **Local development** | Run Flux project in the cloud, Next on `localhost`, and the runner in a terminal—all see the same data. |
...
| **Duplicated configuration** | Both sides need Flux URL + service token (and web OAuth secrets). Keep `.env.example` files aligned. |
...
- **Runner** — Python venv, Flux + `OPENAI_API_KEY`, and CLI commands: [`mailpilot-runner/README.md`](mailpilot-runner/README.md). For dashboard-triggered runs, use **`watch-jobs`** alongside the web app.
...
**Database schema** — [`flux/migrations/`](flux/migrations/) (production). Reference mirror: [`mailpilot-web/supabase/schema.sql`](mailpilot-web/supabase/schema.sql). Apply with `flux push flux/migrations/00N_….sql`.

## Roadmap (snapshot)

- **Done (high level):** Flux data plane, Auth.js + Gmail OAuth, Python worker on Flux, dashboard history and run queue, web undo for processed messages.
